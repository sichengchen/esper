...
test('init — creates all expected directories and files', async () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(tmp, '.esper', 'hooks')))
    ...
    assert.ok(existsSync(join(tmp, '.esper', 'hooks', 'session-reminder.sh')))
    assert.ok(existsSync(join(tmp, '.esper', 'hooks', 'verify-quick.sh')))
...
test('init — generates project hook scripts, not template placeholders', async () => {
  const tmp = await mkdtemp(join(tmpdir(), 'esper-init-test-'))
  try {
    runCLI(['init'], tmp)

    const reminder = await readFile(join(tmp, '.esper', 'hooks', 'session-reminder.sh'), 'utf8')
    assert.ok(reminder.includes('Runs when the host stops'))

    const verifyQuick = await readFile(join(tmp, '.esper', 'hooks', 'verify-quick.sh'), 'utf8')
    assert.ok(verifyQuick.includes('Generated by `esperkit init`'))
    assert.ok(!verifyQuick.includes('This is a template.'))
  } finally {
    await rm(tmp, { recursive: true, force: true })
  }
})
