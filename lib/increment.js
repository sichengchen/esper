...
export async function finish(filename) {
  ...
  if (nextChild) {
    ...
    // Auto-finish the batch parent when the last child completes
    if (frontmatter.parent != null) {
      const activeIncs = await readIncrementsFromDir(join(base, 'active'))
      const parent = activeIncs.find(i => i.id === frontmatter.parent && i.type === 'batch')
      if (parent) {
        const parentContent = await readFile(join(base, 'active', parent._file), 'utf8')
        const { frontmatter: parentFm, body: parentBody } = parseFrontmatter(parentContent)
        parentFm.status = 'done'
        parentFm.finished_at = new Date().toISOString().slice(0, 10)
        await mkdir(join(base, 'done'), { recursive: true })
        await writeFile(join(base, 'done', parent._file), serializeFrontmatter(parentFm, parentBody))
        await unlink(join(base, 'active', parent._file))
      }
    }
