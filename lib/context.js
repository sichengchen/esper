...
export async function build() {
  ...
  // Derive workflow_mode: check old-style default_work_mode first, then fall back
  const workflowMode = wd.default_work_mode ?? 'atom'

  // Derive active_execution_mode from active increment
  let activeExecutionMode = 'interactive'
  if (activeIncrement) {
    const content = await readFile(join(process.cwd(), '.esper', 'increments', 'active', activeIncrement), 'utf8')
    const { frontmatter } = parseFrontmatter(content)
    if (frontmatter.execution_mode) {
      activeExecutionMode = frontmatter.execution_mode
    }
  }

  // Check for active run
  let activeRun = null
  const runsDir = join(process.cwd(), '.esper', 'runs')
  if (existsSync(runsDir)) {
    try {
      const runDirs = await readdir(runsDir)
      for (const dir of runDirs.sort()) {
        const runJson = join(runsDir, dir, 'run.json')
        if (existsSync(runJson)) {
          const runData = JSON.parse(await readFile(runJson, 'utf8'))
          if (runData.status === 'executing' || runData.status === 'review') {
            activeRun = dir
            break
          }
        }
