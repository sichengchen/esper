...
export async function setRoot(newRoot) {
  ...
  await ensureTree(fullPath)
  await updateContext({ spec_root: newRoot })
  console.log(newRoot)
}
...
export async function archive(filePath) {
  ...
  const specRoot = resolveSpecRoot()
  const src = resolveWithin(specRoot, filePath)
  if (!existsSync(src)) {
    console.error(`Spec not found: ${filePath}`)
    process.exit(1)
  }
  const archivedDir = join(specRoot, '_archived')
  const archivedPath = resolveWithin(archivedDir, filePath)
  await mkdir(dirname(archivedPath), { recursive: true })
  const content = await readFile(src, 'utf8')
  const { frontmatter, body } = parseFrontmatter(content)
  frontmatter.status = 'archived'
  const filename = filePath.split('/').pop()
  await writeFile(archivedPath, serializeFrontmatter(frontmatter, body))
...
function resolveWithin(baseDir, targetPath) {
  ...
  if (relPath === '' || relPath === '..' || relPath.startsWith(`..${sep}`)) {
    console.error(`Spec path must stay within ${baseDir}: ${targetPath}`)
    process.exit(1)
  }
  return resolvedPath
}
