...
export async function run() {
  ...
  if (!config.schema_version) {
    ...
    changes.push(`Added schema_version: ${defs.schema_version}`)
  }
  ...
  } else if (config.workflow_defaults.default_work_mode !== undefined || config.workflow_defaults.commit_granularity !== undefined) {
    // Migrate old closed-control workflow_defaults to prompt-style strings
    config.workflow_defaults = defs.workflow_defaults
    changes.push('Migrated workflow_defaults to prompt-style strings')
  }

  // Add new config sections if missing
  if (!config.spec_policy) {
    config.spec_policy = defs.spec_policy
    changes.push('Added spec_policy')
  }
  if (!config.increment_policy) {
    config.increment_policy = defs.increment_policy
    changes.push('Added increment_policy')
  }
  if (!config.agent_roles) {
    config.agent_roles = defs.agent_roles
    changes.push('Added agent_roles')
  }
  if (!config.autonomous_run_policy) {
    config.autonomous_run_policy = defs.autonomous_run_policy
    changes.push('Added autonomous_run_policy')
  }
  if (config.provider_defaults === undefined) {
    config.provider_defaults = defs.provider_defaults
    changes.push('Added provider_defaults')
  }

  // Upgrade schema version to current
  if (config.schema_version < defs.schema_version) {
    config.schema_version = defs.schema_version
    changes.push(`Upgraded schema_version to ${defs.schema_version}`)
  }
  ...
  const runsDir = join(esperDir, 'runs')
  if (!existsSync(runsDir)) {
    await mkdir(runsDir, { recursive: true })
    changes.push('Created runs/')
  }
